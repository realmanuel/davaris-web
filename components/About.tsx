...
>
  ...
  <div className="experience-card" style={{
    ...
    <span style={{ fontFamily: "var(--font-display)", fontSize: "clamp(28px, 5vw, 42px)", fontWeight: 300, color: "var(--black)", lineHeight: 1 }}>2+</span>
    <span style={{ fontFamily: "var(--font-ui)", fontSize: "clamp(7px, 1.5vw, 9px)", letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(0,0,0,0.6)", marginTop: 4 }}>Years of Excellence</span>
  </div>
      ...
      <div>
        ...
        <p className="reveal reveal-delay-2" style={{ fontSize: "clamp(13px, 2vw, 15px)", color: "var(--muted)", lineHeight: 1.8, margin: "clamp(16px, 4vw, 28px) 0 clamp(12px, 3vw, 20px)" }}>
        ...
        <p className="reveal reveal-delay-3" style={{ fontSize: "clamp(13px, 2vw, 15px)", color: "var(--muted)", lineHeight: 1.8 }}>
        ...
        <div className="reveal reveal-delay-3 about-values-grid" style={{
          marginTop: "clamp(24px, 5vw, 40px)", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20,
        }}>
          {values.map((v) => (
            <div key={v.name} style={{ padding: "clamp(12px, 3vw, 20px)", border: "1px solid var(--border)", transition: "border-color 0.3s" }}
              onMouseEnter={(e) => (e.currentTarget.style.borderColor = "rgba(200,169,110,0.3)")}
              onMouseLeave={(e) => (e.currentTarget.style.borderColor = "var(--border)")}
            >
              <div style={{ fontFamily: "var(--font-ui)", fontSize: "clamp(9px, 1.5vw, 11px)", fontWeight: 600, letterSpacing: "0.15em", textTransform: "uppercase", color: "var(--gold)", marginBottom: 8 }}>{v.name}</div>
              <div style={{ fontSize: "clamp(11px, 2vw, 12px)", color: "var(--muted)", lineHeight: 1.6 }}>{v.desc}</div>
            </div>
          ))}
        </div>

        <div className="reveal reveal-delay-4" style={{ marginTop: "clamp(24px, 5vw, 40px)" }}>
